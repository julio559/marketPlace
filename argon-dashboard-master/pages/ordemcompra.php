      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="../pages/tables.html">
            ...
            <span class="nav-link-text ms-1">Ordens de compra</span>
          </a>
        </li>
...
include '../../pages/php/conexao.php';
$id = $_SESSION['usuario'];
$sql = "SELECT * FROM ordemcompra WHERE id_vendedor = $id";
$query = $mysqli -> query($sql);
...
echo '<div class="card-body px-0 pt-0 pb-2">
  <div class="table-responsive p-0">
    <table class="table align-items-center mb-0">
      <thead>
        <tr>
          <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Nome do comprador</th>
          ...
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Nome produto</th>
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Ordem de compra</th>
          <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Id produto</th>
...
while ($row = mysqli_fetch_array($query)) {
    ...
    $id_com = $row['id_cliente'];

    $id_prod = $row['id_prod'];

    $sql23 = "SELECT * FROM clientes WHERE id = $id_com";
    $que23 = $mysqli -> query($sql23);
    while( $row23 = $que23 -> fetch_assoc() ){


$sql22 = "SELECT nome FROM produto WHERE id = $id_prod";
$que22 = $mysqli -> query($sql22);
while( $row22 = $que22 -> fetch_assoc() ){
 $nome_prod = $row22['nome'];   
}
    ...
    echo '<tr>
            <td>
              <div class="d-flex px-2 py-1">
                <div>
                <img src="../../pages/uploads/' . $row23['img_perfil'] . '" class="avatar avatar-sm me-3" alt="avatar">

                </div>
                <div class="d-flex flex-column justify-content-center">
                  <h6 class="mb-0 text-sm">' . $row23['nome'] . '</h6>
   ...
   <td>
              <p class="text-xs font-weight-bold mb-0">' . $row23['cep'] . '</p>
          
            </td>
      

            <td>
              <p class="text-xs font-weight-bold mb-0">' . $row23['cpf'] . '</p>
          
            </td>

            <td>
            <p class="text-xs font-weight-bold mb-0">' . $nome_prod . '</p>
          ...
          <td>
              <p class="text-xs font-weight-bold mb-0">' . $row['id_prod'] . '</p>
          
            </td>
            
            <td>
            <p class="text-xs font-weight-bold mb-0">' . $row['quantidade'] . '</p>
        
          </td>
            <td class="align-middle text-center">
            <span class="text-secondary text-xs font-weight-bold">' . $row23['endereco'] . '</span>
          </td>

          </td>
          <td class="align-middle text-center">
          <a href="chat.php?id=' . $row['id'] . '&id_comprador=' . $row23['id'] . '">
          <span class="text-secondary text-xs font-weight-bold"> Abrir chat </span>
          </a>
        </td>
          </tr>';
}
